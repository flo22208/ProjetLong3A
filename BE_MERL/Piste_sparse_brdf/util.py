...
def subsample_xy(x_input, data, NoL=None, NoV=None, frac=0.5, seed=0):
    rng = np.random.default_rng(seed)
    N = x_input.shape[1]

    if NoL is not None and NoV is not None:
        valid = np.where((NoL >= 0.0) & (NoV >= 0.0))[0]
    else:
        valid = np.arange(N)

    n_keep = max(1, int(len(valid) * frac))
    idx = rng.choice(valid, size=n_keep, replace=False)

    return x_input[:, idx, :], data[:, idx, :]
...
def encode_material_partial(mat, x_grid, NoL, NoV, encoder, size):
    shp = BRDFSHAPE
