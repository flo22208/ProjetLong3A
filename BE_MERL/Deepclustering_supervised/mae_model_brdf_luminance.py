...
# --------------------------------------------------------------------------
# MAE Class - Luminance Version
# --------------------------------------------------------------------------
class MaskedAutoencoderViT3D(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone for 3D Data
    Adapted for variable number of input channels (including single luminance channel)
    """
    def __init__(self, img_size=(90, 90, 180), patch_size=(15, 15, 15), in_chans=1,
                 embed_dim=768, depth=12, num_heads=12,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False, poids=1, norme=2):
        super().__init__()

        # Store in_chans for use in patchify/unpatchify
        self.in_chans = in_chans
        ...
        # Adapter la pondération au nombre de canaux
        self.ponderation = get_ponderation((1, in_chans, 90, 90, 180))
    ...
    def patchify(self, imgs):
        """
        imgs: (N, C, D, H, W) where C = in_chans
        x: (N, L, patch_vol * C)
        """
        ...
        # Reshape to (N, C, Grid_D, Patch_D, Grid_H, Patch_H, Grid_W, Patch_W)
        x = imgs.reshape(shape=(imgs.shape[0], c, n_d, p_d, n_h, p_h, n_w, p_w))
        
        # Permute to (N, Grid_D, Grid_H, Grid_W, Patch_D, Patch_H, Patch_W, C)
        x = torch.einsum('ncdthpwq->ndhptwqc', x)
        
        # Flatten to (N, Num_Patches, Patch_Vol * C)
        x = x.reshape(shape=(imgs.shape[0], n_d * n_h * n_w, p_d * p_h * p_w * c))
        return x
    ...
    def unpatchify(self, x):
        ...
        # Reshape to (N, Grid_D, Grid_H, Grid_W, Patch_D, Patch_H, Patch_W, C)
        x = x.reshape(shape=(x.shape[0], n_d, n_h, n_w, p_d, p_h, p_w, c))
        
        # Permute back to (N, C, Grid_D, Patch_D, Grid_H, Patch_H, Grid_W, Patch_W)
        x = torch.einsum('ndhptwqc->ncdthpwq', x)
        
        # Reshape to final image
        imgs = x.reshape(shape=(x.shape[0], c, n_d * p_d, n_h * p_h, n_w * p_w))
        return imgs
...
def mae_model_brdf_luminance(**kwargs):
    """Factory function for luminance MAE model"""
    model = MaskedAutoencoderViT3D(
        img_size=(90, 90, 180), patch_size=(15, 15, 15), 
        in_chans=1,  # Default à 1 pour la luminance
        embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
...
# set arch
mae_brdf_luminance = mae_model_brdf_luminance
