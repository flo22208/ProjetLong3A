...
def test():
    ...
    ntheta = 90
    nphi_rel = 180
    theta_grid = torch.linspace(0, torch.pi/2, ntheta, dtype=torch.float32)
    phi_grid = torch.linspace(0,torch.pi, nphi_rel, dtype=torch.float32)
    ...
    display_every = 10000000  # Afficher environ 6 exemples sur les 54000 itérations
    iteration_count = 0

    mat_res = torch.zeros((90, 180, 90, 1), dtype=torch.int32)
    
    for i in range(ntheta):
        print(f"Test {i+1}/{ntheta} : θi = {theta_grid[i].item():.3f} rad")
        for k in range(nphi_rel):
            theta_i = theta_grid[i]
            theta_o = theta_grid[i]
            phi_i = phi_grid[k]/2
            phi_o =  -phi_grid[k]/2  # pour garantir φh = 0
            ...
            ## id
            theta_idx = int(torch.clamp((theta_i / (torch.pi/2)) * (90 - 1), 0, 90 - 1).item())
            theta_d_idx =   int(torch.clamp((theta_d / (torch.pi)) * (180- 1), 0, 180 - 1).item())
            phi_d_idx =     int(torch.clamp((phi_d / torch.pi/2) * (90 - 1), 0, 90 - 1).item())
            mat_res[theta_idx, theta_d_idx, phi_d_idx, 0] = 1
            
            # Afficher la visualisation 3D tous les N itérations
            if iteration_count % display_every == 0:
                iteration_info = f"Itération {iteration_count}/{nombre_tests} (i={i}, k={k})"
    ...
    print("nombre de valeurs atteintes : ", mat_res.sum().item(),"/", mat_res.numel())
