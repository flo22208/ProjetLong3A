...
try:
    from mae_model_brdf_luminance import MaskedAutoencoderViT3D
except ImportError:
    print("Warning: mae_model_brdf_luminance not found")
...
if __name__ == "__main__":
    ...
    lr_after_warmup_mae = 1e-4          # Learning rate après warmup pour MAE
    ...
    cluster_loss_weight_after_warmup = 0.2  # Poids de la
    ...
    seed = 0                         # Graine aléatoire
     ...
     # Configuration
    print("="*70)
    print("MAE + CLUSTERING TRAINING ON MERL (LUMINANCE ONLY)")
    print("="*70)
    ...
    class Args:
        def __init__(self):
            ...
            self.materiau = materiaux
            self.luminance_only = True  # Activer le mode luminance uniquement
    ...
    if MaskedAutoencoderViT3D is None:
        raise ImportError("mae_model_brdf_luminance not available")
    
    mae = MaskedAutoencoderViT3D(
        in_chans=1,  # 1 canal pour la luminance au lieu de 3 pour RGB
        poids=poids,
